...
module FlatApi
  class LtiConfigurationBase < ApiModelBase
    # Configuration ID
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FlatApi::LtiConfigurationBase` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FlatApi::LtiConfigurationBase`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      lti_version_validator = EnumAttributeValidator.new('String', ["1p1", "1p3"])
    ...
    def lti_version=(lti_version)
      validator = EnumAttributeValidator.new('String', ["1p1", "1p3"])
    ...
    def hash
      [id, lti_version, organization_id, organization_name, creator_id, creation_date, last_used_date, status].hash
    end
