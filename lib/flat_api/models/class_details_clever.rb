...
module FlatApi
  ...
  class ClassDetailsClever < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FlatApi::ClassDetailsClever` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FlatApi::ClassDetailsClever`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'term_name')
        self.term_name = attributes[:'term_name']
      end

      if attributes.key?(:'term_start_date')
        self.term_start_date = attributes[:'term_start_date']
      end

      if attributes.key?(:'term_end_date')
        self.term_end_date = attributes[:'term_end_date']
      end
    end
    ...
    def hash
      [id, creation_date, modification_date, subject, term_name, term_start_date, term_end_date].hash
    end
