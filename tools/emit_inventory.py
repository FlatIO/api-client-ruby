#!/usr/bin/env python3
"""Emit OPERATIONS.json for the ruby SDK (FR-001, FR-002).
...
def main() -> None:
    ...
    for path in (ROOT / "lib" / "flat_api" / "api").glob("*.rb"):
    ...
    models = set()
    for path in (ROOT / "lib" / "flat_api" / "models").glob("*.rb"):
        text = path.read_text()
        models |= set(re.findall(r"^  (?:class|module) (\w+)\b", text, re.M))
    ...
    inventory = {
        ...
        "generator": {"name": "ruby", "version": "7.24.0"},
