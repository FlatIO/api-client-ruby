#!/usr/bin/env bash
# Regenerate the Flat Ruby SDK from the public OpenAPI specification.
#
...
mkdir -p lib docs/reference .openapi-generator
cp -R "$SCRATCH/out/lib/." lib/
...
p = pathlib.Path("lib/flat_api/version.rb")
p.parent.mkdir(parents=True, exist_ok=True)
t = p.read_text() if p.is_file() else "module FlatApi\n  VERSION = '0.0.0'\nend\n"
p.write_text(re.sub(r"VERSION = '[0-9.]+'", f"VERSION = '{sys.argv[1]}'", t))
VW
